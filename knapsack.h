...
void knapsack::insertionSort(){
        int i, j, valkey, costkey, numLength = numObjects;
        double key;
            for(j = 1; j < numLength; j++)    // Start with 1 (not 0)
            {
                ...
                for(i = j - 1; (i >= 0) && (ratio[i] < key); i--)   // Smaller values move up
                {
                    ratio[i+1] = ratio[i];
                    value[i+1] = value[i];
                    cost[i+1] = cost[i];
                }
                ratio[i+1] = key; //Put key into its proper location
                value[i+1] = valkey;
                cost[i+1] = costkey;
            }
            return;
